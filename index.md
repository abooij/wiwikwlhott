...
Coq consists of several layers of programming language.  The most
famous one is the tactic language that is used to prove theorems.  But
since theorems are interpreted by types, really tactics just construct
terms.  For example:
...
* minor gotcha: even if your particular flavor of hott is strongly
  ...
  there are still types without decidable equality, because there is
  no map inside type theory that computes normal forms of arbitrary
  terms.  find a way to say this without hopping back and forth
  between meta-theoretical and internal statements.
